...
<%-- <%
	...
	String state = "../images/dong/map0.png";
	String getState ="도/시";
	String getCity ="구/군";
	...
	if(request.getParameter("city")!=null){
		String state1 = request.getParameter("selectState");
		String city = request.getParameter("city");
		String address = state1+" "+city;
		getCity = city;
		APTnameList = aptService.getAptNameService(address);
		 
		
	}

	
	if(request.getParameter("selectState")!=null){
	getState = request.getParameter("selectState");
		if (getState.equals("제주")) {
	state = "../images/dong/map1.png";
		} else if (getState.equals("울산")) {
	state = "../images/dong/map2.png";
		} else if (getState.equals("부산")) {
	state = "../images/dong/map3.png";
		} else if (getState.equals("경남")) {
	state = "../images/dong/map4.png";
		} else if (getState.equals("대구")) {
	state = "../images/dong/map5.png";
		} else if (getState.equals("경북")) {
	state = "../images/dong/map6.png";
		} else if (getState.equals("광주")) {
	state = "../images/dong/map7.png";
		} else if (getState.equals("전남")) {
	state = "../images/dong/map8.png";
		} else if (getState.equals("전북")){
	state = "../images/dong/map9.png";
		} else if (getState.equals("대전")) {
	state = "../images/dong/map10.png";
		} else if (getState.equals("충북")) {
	state = "../images/dong/map11.png";
		} else if (getState.equals("충남")) {
	state = "../images/dong/map12.png";
		} else if (getState.equals("강원")) {
	state = "../images/dong/map13.png";
		} else if (getState.equals("경기")) {
	state = "../images/dong/map14.png";
		} else if (getState.equals("인천")) {
	state = "../images/dong/map15.png";
		} else if (getState.equals("서울")) {
	state = "../images/dong/map16.png";
		} else {
	state = "../images/dong/map0.png";
		}
		
		
		cityList = aptService.getCityService(getState);
		request.setAttribute("cityList", cityList);
	}
	
	List<AddressState> list = aptService.getStateService();
	 
	request.setAttribute("list", list);
	request.setAttribute("state", state);
	request.setAttribute("cityList", cityList);
	request.setAttribute("getState", getState);
	request.setAttribute("getCity", getCity);
	request.setAttribute("APTnameList", APTnameList);
%> --%>
...
<script type="text/javascript" src="/resources/dist/js/dong/jquery-1.11.0.js"></script>
...
<body>
	...
	<div  style="background: white; padding: 0px; height: 400px;" >
		...
		<div>
			<img id="mapImage" src="<%-- ${state } --%>/resources/images/dong/map0.png" usemap="#interior"
				style="float: left;">
		</div>
		<div>
			<map name="interior">
		...
		<div style="margin-top: 40px ">
			<form action="map" name="chooseState" method="post">
				...
				<select name="selectState" style="width: 120px">

					<option id="select1" selected="selected">${getState }</option>

					<c:forEach var="m" items="${stateList}">

						<option onclick="fncRegnCd('${m.getAddr_state()}')">${m.getAddr_state()}</option>

					</c:forEach>
				</select> 
				
				<input type="submit" value="도/시 선택" style="width: 120px">
				
			</form>
			
			<form action="map" name="chooseState" method="post">
				<input type="hidden" name="selectState" value="${getState }">
				<select name="city" style="width: 120px">
					<option id="select2">${getCity }</option>
					<c:forEach var="l" items="${cityList}">
						<option>${l.getAddr_city()}</option>
					</c:forEach>
				</select> <input type="submit" value="구/군선택" style="width: 120px">
			</form> 




			<form action="signup" name="apt" method="get">
				<select name="aptname" style="width: 120px">

					<option id="APTName" selected="selected">아파트선택</option>

					<c:forEach var="a" items="${APTnameList}">
				...
				<input type="button" value="아파트 선택" onclick="aaa(this.form)"
					style="width: 120px">
			</form>
		</div>
