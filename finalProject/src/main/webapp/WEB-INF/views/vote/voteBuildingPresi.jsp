...
//-----------------  manager's code	-----------------------------	
	...
	if(vf != null){
		...
		aptService.updateV2flagService(changeVflag, "tndusdla92");
	}
	...
	if(m.getM_grade() == 2){	
		if(changeVflag == 1){
			...
			aptService.updateAllResiV2flagService(hash);
			aptService.updateV2flagService(changeVflag, "tndusdla92");
			...
			String newGroupPresi = aptService.selectTopBuildingCandiService(m.getApt_APTGNo());
			aptService.updateBPGradeService(newGroupPresi);
		}
	}
	...
	int voterNum = aptService.totalBPVoterNum(m.getApt_APTGNo());
...
//------------------- resident's code ------------------------------------
	Member gp = aptService.selectBuildingPresiService(m.getApt_APTGNo(),m.getM_buildingNo());
...
<body>
	...
	<c:choose>
		<c:when test="${m.getM_grade() == 2 }"> <!-- case: manager -->
			<c:choose>
				<c:when test="${m.getV2_flag() == 1 }">
					<form class="vote_back" action="voteBuildingPresiRegists.jsp" method="post">
				...
				<c:when test="${m.getV2_flag() == 3 }">
					...
					<div class="col-md-8">
						<form action="voteBuildingPresi.jsp?changVflag=4" method="post">
							<div class="progress">
		...
		<c:otherwise>
			...
			<c:choose>
				<c:when test="${m.getV2_flag() == 1 }">		
					<br><h2>${m.getM_buildingNo()}동의 동대표</h2><br><br>
					...
					<c:forEach var="br" items="${vlist }">
						<c:if test="${m.m_buildingNo == br.v_buildingNo }">
							<c:if test="${m.m_roomNo == br.v_roomNo }">
								<% response.sendRedirect("voteBuildingPresi.jsp?changeVflag=3"); %>
							</c:if>
						</c:if>
					</c:forEach>
					<br><h2>동대표 선거가 진행중입니다.</h2><br><br>
